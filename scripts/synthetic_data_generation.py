...
class SyntheticExample(BaseModel):
    ...
    excerpt: str = Field(description="Exact text excerpt from the document that can be used to answer the question")
...
Generate a factual question about this document and identify the exact text excerpts that answer it with necessary context.

Requirements:
1. Question must be specific and answerable ONLY from the provided text
2. Excerpts must be EXACT word-for-word copies from the document. No summerization or paraphrasing
3. Keep excerpts concise (1-3 sentences each) with neccessary context
4. Avoid questions with the word "and" unless it's part of a proper noun
